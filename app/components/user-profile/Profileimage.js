  ...
  const handleUploadImage = async () => {
    if (selectedImage) {
      try {
        ...
        const fileData = await FileSystem.readAsStringAsync(selectedImage.assets[0].uri, {
      ...
      //   console.log(response1);
        ...
        const response2 = await Raxios.get(`/userProfile/profilePic/${user.userId}/${user.gymId}`);
